...
session_root = fullfile(mouse_root, 'AH1473_08_08_25_WNoiseWater_AutoTrain', 'CtxImg');
...
tm_opts = struct();
tm_opts.behavior_predictor = 'Face';               % Face trace from face_ROI.roimsk
tm_opts.target_neural_rois = ROI.modalities.fluorescence.labels;  % or subset
tm_opts.min_lag_seconds = -2.0;                    % allow neural leads
tm_opts.max_lag_seconds = 4;                     % allow neural lags
tm_opts.cv_folds = 5;
tm_opts.show_plots = true;      % still need plots enabled
tm_opts.poster_plots = true;    % switch to poster layout
tm_opts.no_category_plots = true; % skip predictive/reactive categories in plots
tm_opts.prediction_rois = {'BC_L','V1_L'};  % pick ROIs for the prediction panels

%% Run full temporal model
results = TemporalModelFull(ROI, tm_opts);
