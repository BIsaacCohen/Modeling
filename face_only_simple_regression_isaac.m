function results = face_only_simple_regression_isaac(fluo_file, motion_file, behav_roi_file, neural_roi_file, vascular_mask_file, brain_mask_file, opts)
% face_only_simple_regression_isaac Simple OLS regression of face motion -> fluorescence
%
%   Simplified version that performs ordinary least squares regression to
%   predict fluorescence from face motion only (no stimulus or lick regressors).
%   Uses single fit on all data (no cross-validation).
%
%   results = face_only_simple_regression_isaac(fluo_file, motion_file, ...
...
%       beta                - Regression coefficient for face motion
%       intercept           - Regression intercept
%       R2                  - R-squared on all data
%       Y_pred              - Predicted fluorescence trace
%       neural_trace        - Original fluorescence trace (z-scored)
%       face_motion         - Face motion trace (z-scored)
%       regressor_name      - 'face_motion'
...
defaults = struct('target_roi_name', 'AU_L', 'chunk_size_frames', 500, ...
    'output_file', '', 'motion_smoothing_window', 3);
...
if isempty(opts.output_file)
    ...
    default_output = sprintf('face_only_isaac_%s_results.mat', safe_roi);
...
if isempty(roi_idx)
    error('ROI "%s" not found in %s.\nAvailable: %s', ...
        opts.target_roi_name, roi_source_path, strjoin(roi_names, ', '));
end
...
%% 9. OLS regression: Y = beta * X + intercept
fprintf('Performing OLS regression...\n');
X = face_motion_z;
Y = neural_trace_z;
...
fprintf('  Beta (face motion coefficient): %.4f\n', beta);
fprintf('  R^2: %.4f (%.2f%%)\n', R2, R2 * 100);

%% 10. Save results
results = struct();
results.target_roi_name = opts.target_roi_name;
results.regressor_name = 'face_motion';
results.beta = beta;
...
results.paths = struct('fluo_file', fluo_file, 'motion_file', motion_file, ...
    'behav_roi_file', behav_roi_file, 'neural_roi_file', neural_roi_file);
...
function log_cfg(fluo_file, motion_file, neural_roi_file, opts)
    fprintf('=== Face-only simple OLS regression configuration ===\n');
