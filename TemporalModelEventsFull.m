function results = TemporalModelEventsFull(ROI, session_file, opts)
% TemporalModelEventsFull  Multi-ROI temporal model with motion + event predictors.
%
%   Fits the combined lagged motion predictor and event-regressor model
%   (TemporalModelEvents) across ALL selected neural ROIs simultaneously.
%   This mirrors TemporalModelFull for efficiency while preserving
%   identical lag, event-kernel, and cross-validation behavior.
%
%   results = TemporalModelEventsFull(ROI, session_file)
...
fprintf('=== TemporalModelEventsFull: multi-ROI motion+event regression ===\n');
...
fprintf('=== TemporalModelEventsFull complete ===\n');
end
%% ================= Helper functions =================
