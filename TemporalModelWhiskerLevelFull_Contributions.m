function results = TemporalModelWhiskerLevelFull_Contributions(ROI, session_file, opts)
% TemporalModelWhiskerLevelFull_Contributions_v2  Motion + whisker-level multi-ROI model with contributions.
%
%   Mirrors TemporalModelWhiskerLevelFull but additionally computes per-group
%   explained and unique contributions (single-variable fits + shuffle drops)
%   for the motion predictor, each whisker-level block, and each lick block.
%
%   results = TemporalModelWhiskerLevelFull_Contributions_v2(ROI, session_file)
%   results = TemporalModelWhiskerLevelFull_Contributions_v2(ROI, session_file, opts)
%
...
fprintf('=== TemporalModelWhiskerLevelFull_Contributions_v2: multi-ROI motion+whisker-level regression ===\n');
t_start = tic;
...
if opts.save_results
    ...
    if isempty(opts.output_file)
        default_file = sprintf('TemporalModelWhiskerLevelFull_Contributions_v2_%s_%s_%s.mat', ...
...
fprintf('=== TemporalModelWhiskerLevelFull_Contributions_v2 complete ===\n');
end
...
if any(conv_fail) && isfinite(time_limit)
    warning('TemporalModelWhiskerLevelFull_Contributions_v2:RidgeTimeout', ...
...
if any(exceed_mask)
    warning('TemporalModelWhiskerLevelFull_Contributions_v2:LambdaCap', ...
...
whisker_cfg = get_whisker_level_info();
design_matrix = [];
regressor_names = {};
column_group = [];
column_lags = [];
...
for s = 1:numel(whisker_cfg.field_names)
    field_name = whisker_cfg.field_names{s};
    group_defs(end+1).label = whisker_cfg.labels{s}; %#ok<AGROW>
    group_defs(end).prefix = whisker_cfg.prefixes{s};
    group_idx = numel(group_defs);

    level_times = getfield_or_empty(stimulus_events, field_name);
    level_intensity = ones(size(level_times));
    whisker_regressors = create_stimulus_regressors( ...
        level_times, level_intensity, n_timepoints, sampling_rate, stim_kernel);
    design_matrix = [design_matrix, whisker_regressors]; %#ok<AGROW>
...
for s = 1:numel(whisker_cfg.field_names)
    ...
    noise_intensities_all = [noise_intensities_all; repmat(whisker_cfg.level_values(s), numel(values), 1)]; %#ok<AGROW>
end
...
noise_intensities = [];
for s = 1:numel(whisker_cfg.field_names)
    onset_field = sprintf('%s_onsets', whisker_cfg.field_names{s});
    if isfield(events, onset_field)
        ...
        noise_intensities = [noise_intensities; repmat(whisker_cfg.level_values(s), numel(trimmed_times), 1)]; %#ok<AGROW>
    end
end
...
whisker_total = 0;
for s = 1:numel(whisker_cfg.field_names)
    counts.(whisker_cfg.field_names{s}) = 0;
end
...
for s = 1:numel(whisker_cfg.field_names)
    onset_field = sprintf('%s_onsets', whisker_cfg.field_names{s});
    if isfield(events, onset_field)
        counts.(whisker_cfg.field_names{s}) = numel(events.(onset_field));
        whisker_total = whisker_total + counts.(whisker_cfg.field_names{s});
    end
end
if isfield(events, 'noise_onsets')
    counts.noise_primary = numel(events.noise_onsets);
else
    counts.noise_primary = whisker_total;
end
...
function cfg = get_whisker_level_info()
cfg = struct();
cfg.level_values = [1, 5, 25, 125];
cfg.field_names = arrayfun(@(ms) sprintf('whisker%dms', ms), cfg.level_values, 'UniformOutput', false);
cfg.prefixes = cellfun(@(name) sprintf('%s_', name), cfg.field_names, 'UniformOutput', false);
cfg.labels = arrayfun(@(ms) sprintf('Whisker %d ms stimulus', ms), cfg.level_values, 'UniformOutput', false);
cfg.event_names = {'Whisker1ms','Whisker5ms','Whisker25ms','Whisker125ms'};
cfg.event_ids = containers.Map(cfg.event_names, cfg.field_names);
end

function lookup = build_whisker_event_lookup(event_names)
if isstring(event_names)
    event_names = cellstr(event_names);
end
cfg = get_whisker_level_info();
lookup = repmat({''}, numel(event_names), 1);
for i = 1:numel(event_names)
    if isempty(event_names{i})
        continue;
    end
    lookup{i} = match_whisker_level_field_from_event_name(event_names{i});
    if isempty(lookup{i})
        token = regexp(event_names{i}, 'Whisker\s*(?<val>\d+(\.\d+)?)', 'names', 'once');
        if ~isempty(token)
            lookup{i} = match_whisker_level_field_from_intensity(str2double(token.val));
        end
    end
end
end

function field_name = determine_session_whisker_field(SessionData, trial_idx)
field_name = '';
intensity = get_whisker_level_value_from_session(SessionData, trial_idx);
if ~isnan(intensity)
    field_name = match_whisker_level_field_from_intensity(intensity);
end
end

function intensity = get_whisker_level_value_from_session(SessionData, trial_idx)
cfg = get_whisker_level_info();
intensity = NaN;
if isfield(SessionData, 'TrialData') && numel(SessionData.TrialData) >= trial_idx
    trial_entry = SessionData.TrialData(trial_idx);
    candidate_fields = {'VolumeLevel','WhiskerLevel','NoiseLevel','StimulusLevel'};
...
if isnan(intensity) && isfield(SessionData, 'TrialSettings')
    ...
    candidate_fields = {'WhiskerLevel','WhiskerDuration','WhiskerAmplitude','TargetDuration','TargetLevel'};
...
if isnan(intensity) && isfield(SessionData, 'Custom') && ~isempty(SessionData.Custom)
    ...
    candidate_fields = {'WhiskerLevel','WhiskerDuration','WhiskerLevels','TargetDuration'};
    for c = 1:numel(candidate_fields)
        field = candidate_fields{c};
...
if isnan(intensity) && isfield(SessionData, 'WhiskerLevels')
    intensity = pick_trial_value(SessionData.WhiskerLevels, SessionData, trial_idx);
end
end
...
if isstruct(entry_val)
    if isfield(entry_val, 'Index') && ~isempty(entry_val.Index)
        ...
        if idx_val >= 1 && idx_val <= numel(cfg.level_values)
            val = cfg.level_values(idx_val);
            return;
        end
    end
    nested_fields = {'Duration','WhiskerLevel','WhiskerDuration','StimulusLevel','Amplitude'};
...
function field_name = match_whisker_level_field_from_intensity(intensity)
field_name = '';
...
cfg = get_whisker_level_info();
[diff_min, idx] = min(abs(cfg.level_values - val));
if diff_min <= 1e-6
    field_name = cfg.field_names{idx};
else
    persistent warned_invalid_intensity;
    if isempty(warned_invalid_intensity)
        warning('Whisker duration %.3f does not match configured whisker levels.', double(intensity));
        warned_invalid_intensity = true;
    end
end
end

function field_name = match_whisker_level_field_from_event_name(event_name)
field_name = '';
...
token = regexp(event_name, 'Whisker\s*(?<val>\d+(\.\d+)?)', 'names', 'once');
if ~isempty(token)
    field_name = match_whisker_level_field_from_intensity(str2double(token.val));
end
end
...
for s = 1:numel(whisker_cfg.field_names)
    stimulus_events.(whisker_cfg.field_names{s}) = [];
end
lick_events = struct();
...
for trial = 1:n_trials
    ...
    if isfield(states, 'NoiseDelivery') && ~isempty(states.NoiseDelivery)
        level_field = determine_session_whisker_field(SessionData, trial);
...
whisker_cfg = get_whisker_level_info();
whisker_lookup = build_whisker_event_lookup(event_names);
...
for i = 1:numel(timestamps)
    ...
    time = timestamps(i);
    whisker_field = '';
    if id <= numel(whisker_lookup)
        whisker_field = whisker_lookup{id};
    end
    if ~isempty(whisker_field)
        stimulus_events.(whisker_field)(end+1,1) = time; %#ok<AGROW>
...
n_rois = numel(tk);
colors = lines(n_rois);
figure('Name','TemporalModelWhiskerLevelFull_Contributions_v2 Kernels','Position',[80 80 900 500]);
hold on;
for r = 1:n_rois
...
figure('Name','TemporalModelWhiskerLevelFull_Contributions_v2 Heatmap','Position',[100 100 700 500]);
...
figure('Name','TemporalModelWhiskerLevelFull_Contributions_v2 Performance','Position',[120 120 800 400]);
