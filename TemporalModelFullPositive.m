function results = TemporalModelFullPositive(ROI, opts)
% TemporalModelFullPositive Multi-ROI bidirectional lag ridge regression (positive peaks only)
%
%   Fits temporal models for ALL neural ROIs simultaneously:
%   Y_i(t) = Î£ Î²_i,jÂ·X(t-j) with bidirectional lags (leads and lags)
...
%       temporal_kernels    - [n_ROIs Ã— 1] struct array with fields:
%           .roi_name           - Name of this ROI
%           .beta_cv_mean       - CV-averaged beta coefficients [n_lags Ã— 1]
%           .beta_cv_sem        - Standard error across CV folds
%           .beta_full_data     - Beta from full-data fit
%           .beta_cv_folds      - Beta weights from each CV fold [n_lags Ã— n_folds]
...
%       performance         - [n_ROIs Ã— 1] struct array with fields:
%           .roi_name           - Name of this ROI
%           .R2_cv_mean         - Mean RÂ² across CV folds (realistic estimate)
%           .R2_cv_sem          - Standard error of CV RÂ²
%           .R2_cv_folds        - RÂ² for each fold [1 Ã— n_folds]
%           .R2_full_data       - RÂ² from full-data fit (optimistic)
...
%           .roi_names          - {1 Ã— n_ROIs} cell array of names
%           .beta_matrix_cv     - [n_lags Ã— n_ROIs] all CV-averaged kernels
%           .R2_all_rois        - [1 Ã— n_ROIs] RÂ² for each ROI
%           .peak_lags_all_sec  - [1 Ã— n_ROIs] peak lag for each ROI
%           .peak_betas_all     - [1 Ã— n_ROIs] peak beta for each ROI
%       predictions         - Model predictions (from full-data model)
%           .Y_pred             - [n_valid Ã— n_ROIs] predicted fluorescence
%           .Y_actual           - [n_valid Ã— n_ROIs] actual z-scored fluorescence
...
fprintf('=== TemporalModelFullPositive: Multi-ROI bidirectional lag ridge regression (positive peaks) ===\n');
...
% Y is now [n_valid Ã— n_rois] - ALL ROIs together
Y_all = neural_traces_z(max_lag+1 : min_length-abs(min_lag), :);

fprintf('  Design matrix X: [%d Ã— %d] (timepoints Ã— lags)\n', size(X, 1), size(X, 2));
fprintf('  Outcome matrix Y: [%d Ã— %d] (timepoints Ã— ROIs)\n', size(Y_all, 1), size(Y_all, 2));
...
if condition_num > 30
    fprintf('  â†’ High collinearity expected (using ridge regression)\n');
end
...
beta_cv_folds = zeros(n_lags_total, n_rois, cv_folds);  % [lags Ã— ROIs Ã— folds]
lambda_cv_folds = zeros(n_rois, cv_folds);              % [ROIs Ã— folds]
R2_cv_folds = zeros(n_rois, cv_folds);                  % [ROIs Ã— folds]
convergence_cv = zeros(n_rois, cv_folds);               % [ROIs Ã— folds]
...
for fold = 1:cv_folds
    ...
    % ridgeMML returns [n_predictors Ã— n_outcomes] betas and [1 Ã— n_outcomes] lambdas
    [lambda_fold, beta_fold, conv_fail] = ridgeMML(Y_train_all, X_train, 1);

    % Store results for each ROI
    beta_cv_folds(:, :, fold) = beta_fold;      % [n_lags Ã— n_rois]
    lambda_cv_folds(:, fold) = lambda_fold';    % [n_rois Ã— 1]
    convergence_cv(:, fold) = conv_fail';       % [n_rois Ã— 1]

    % Predict on test data for all ROIs
    Y_pred_test_all = X_test * beta_fold;       % [n_test Ã— n_rois]

    % Compute test RÂ² for each ROI independently
    for roi = 1:n_rois
    ...
    fprintf('  Fold %d/%d: RÂ² range [%.4f, %.4f], mean lambda = %.4f\n', ...
        fold, cv_folds, min(R2_cv_folds(:, fold)), max(R2_cv_folds(:, fold)), ...
        mean(lambda_fold));
end

% Compute CV statistics per ROI
R2_cv_mean = mean(R2_cv_folds, 2);              % [n_rois Ã— 1]
R2_cv_sem = std(R2_cv_folds, 0, 2) / sqrt(cv_folds);  % [n_rois Ã— 1]
lambda_cv_mean = mean(lambda_cv_folds, 2);     % [n_rois Ã— 1]

fprintf('\nCV Results Summary:\n');
fprintf('  RÂ² (CV mean) range: [%.4f, %.4f]\n', min(R2_cv_mean), max(R2_cv_mean));
fprintf('  Best ROI: %s (RÂ² = %.4f Â± %.4f)\n', ...
    roi_names{R2_cv_mean == max(R2_cv_mean)}, max(R2_cv_mean), R2_cv_sem(R2_cv_mean == max(R2_cv_mean)));
fprintf('  Worst ROI: %s (RÂ² = %.4f Â± %.4f)\n', ...
...
lambda_full = lambda_full';  % [n_rois Ã— 1]
...
Y_pred_full = X * beta_full;  % [n_valid Ã— n_rois]

% Compute full-data RÂ² for each ROI
...
fprintf('  RÂ² (full-data) range: [%.4f, %.4f]\n', min(R2_full), max(R2_full));
...
beta_cv_mean_all = mean(beta_cv_folds, 3);  % [n_lags Ã— n_rois]
beta_cv_sem_all = std(beta_cv_folds, 0, 3) / sqrt(cv_folds);  % [n_lags Ã— n_rois]
...
for roi = 1:n_rois
    kernel = beta_cv_mean_all(:, roi);
    kernel_sem = beta_cv_sem_all(:, roi);
    kernel_positive = kernel;
    kernel_positive(kernel_positive <= 0) = -inf;
    [peak_beta_pos, peak_idx] = max(kernel_positive);
    if isfinite(peak_beta_pos)
        peak_lags_frames(roi) = lag_values(peak_idx);
        peak_lags_sec(roi) = lag_times_sec(peak_idx);
        peak_betas(roi) = kernel(peak_idx);
        peak_betas_sem(roi) = kernel_sem(peak_idx);
    else
        peak_lags_frames(roi) = NaN;
        peak_lags_sec(roi) = NaN;
        peak_betas(roi) = NaN;
        peak_betas_sem(roi) = NaN;
    end
end
...
for roi = 1:n_rois
    ...
    results.temporal_kernels(roi).beta_cv_folds = squeeze(beta_cv_folds(:, roi, :));  % [n_lags Ã— n_folds]
...
results.comparison.beta_matrix_cv = beta_cv_mean_all;       % [n_lags Ã— n_rois]
results.comparison.R2_all_rois = R2_cv_mean';               % [1 Ã— n_rois]
results.comparison.peak_lags_all_sec = peak_lags_sec';      % [1 Ã— n_rois]
results.comparison.peak_betas_all = peak_betas';            % [1 Ã— n_rois]

% --- Predictions (from full-data model) ---
results.predictions = struct();
results.predictions.Y_pred = Y_pred_full;                   % [n_valid Ã— n_rois]
results.predictions.Y_actual = Y_all;                       % [n_valid Ã— n_rois]
...
results.metadata.peak_detection = 'positive_only';
...
%% 12. Generate plots
if opts.show_plots
    PlotTemporalModelFullPositive(results);
end
...
if opts.save_results
    if isempty(opts.output_file)
        ...
        default_output = sprintf('TemporalModelFullPositive_%dROIs_vs_%s.mat', n_rois, safe_behav);
...
fprintf('\n=== TemporalModelFullPositive complete ===\n');
